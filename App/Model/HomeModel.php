...
class HomeModel extends Database{
    ...
    function homeConnect(){
        ...
        $sond = $this->query(" SELECT q.`question`, u.`pseudo`, q.`image`, q.`date_fin` FROM `question` as q INNER JOIN `user` as u on q.`user_id_author` = u.`id` WHERE date_fin >= CURDATE()  AND `user_id_author` <> '$membre_id'  ORDER BY date_fin ASC");
